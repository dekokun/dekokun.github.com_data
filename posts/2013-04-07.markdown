...
## developmentの際とproductionの際にデプロイ先を分けたい

まぁ、デプロイツールであれば当然の要求ですよね

### 解決策

方法がいくつか考えられます。タスクを分ける方法と、タスクのなかでホストを定義する方法。(その他もろもろ)
...
他にもfab deploy:devserver など、コマンドラインでデプロイ先をタスクに直接与える方法や、デフォルトのfab -Hでデプロイ先を指定する方法など、いろいろ考えられますね。

私は、fabfileでデプロイ先をバージョン管理したいかつ、一番柔軟な方法を取りたいという思いがあり、上記の「タスクのなかでホストを定義」を採用しています。

## ロールを分けたい

production環境、develop環境でも、Webサーバやツールサーバなど、複数のロールによってデプロイ先を切り替えるなど行いたい欲求は確実に出てきます。Capistranoのロールのようなものが欲しいですよね。
...
$ fab hello:name=Jeff
とかね。最初に提示したチュートリアルに載っているから詳しくはそっち参考にしてね。
...
$ fab -Pで、全てのタスクを並列実行にすることが可能です。

タスク毎に分ける場合は、以下のようにデコレータを使用してください。
...
ここ、本当にこれでいいんですかね。Fabric使いの人からのツッコミをお待ちしております。
